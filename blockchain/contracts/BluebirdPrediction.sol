...
/**
 * @title BluebirdPrediction
 */
contract BluebirdPrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    address public bluebird; // address of the bluebird token
     ...
     * @param epoch: epoch
     */
    function betBearETH(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
     ...
     * @param epoch: epoch
     */
    function betBullETH(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
     ...
     * @notice Bet bear position
     * @param _epoch: epoch
     * @param _amount: amount of token to bet
     */
    function betBear(uint256 _epoch, uint256 _amount) external whenNotPaused nonReentrant notContract {
        require(_epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(_epoch), "Round not bettable");
        require(_amount >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[_epoch][msg.sender].amount == 0, "Can only bet once per round");
        require(IERC20(bluebird).transferFrom(msg.sender, address(this), _amount), "Transfer token failed");
        // Update round data
        ...
        emit BetBear(msg.sender, _epoch, _amount);
    }
     ...
     * @param _amount: amount of token to bet
     */
    function betBull(uint256 _epoch, uint256 _amount) external payable whenNotPaused nonReentrant notContract {
        require(_epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(_epoch), "Round not bettable");
        require(_amount >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[_epoch][msg.sender].amount == 0, "Can only bet once per round");
        ...
        emit BetBull(msg.sender, _epoch, _amount);
    }
