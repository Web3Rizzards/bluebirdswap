...
contract OptionPricing is Ownable {
    ...
    function getOptionPrice(
        ...
        uint256 timeToExpiry = (expiry - block.timestamp) / 864;

        return
            BlackScholes.calculate(
    ...
    function getIV(
        ...
        if (strike > lastPrice) {
            percentageDifference = (uint256(100) * (strike - lastPrice) * strikePrecision) / lastPrice;
        } else {
            percentageDifference = (uint256(100) * (lastPrice - strike) * strikePrecision) / lastPrice;
        }

        percentageDifference = percentageDifference / strikePrecision;

        // timeToExpiry in days = timestamp (in sec) / 86400
        bytes16 timeToExpiry = ABDKMathQuad.div(
            ABDKMathQuad.fromUInt(expiry - block.timestamp),
        ...
        bytes16 exponent = ABDKMathQuad.div(
            ...
            ABDKMathQuad.mul(
                ABDKMathQuad.div(ABDKMathQuad.fromUInt(curveMultiplier), ABDKMathQuad.fromUInt(1e8)),
                timeToExpiry
            )
        );
        ...
        bytes16 ivMultiple = ABDKMathQuad.add(
            ...
            ABDKMathQuad.mul(
                ABDKMathQuad.mul(
                    ABDKMathQuad.div(ABDKMathQuad.fromUInt(growthMultiplier), ABDKMathQuad.fromUInt(1e8)),
        ...
        uint256 iv = ABDKMathQuad.toUInt(ABDKMathQuad.mul(ABDKMathQuad.fromUInt(baseIv), ivMultiple));
